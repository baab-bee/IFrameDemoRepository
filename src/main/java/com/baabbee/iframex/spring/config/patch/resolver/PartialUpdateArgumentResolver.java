package com.baabbee.iframex.spring.config.patch.resolver;
...
//        Object id = idClass.cast(idStr);
        String id = getPathVariables(webRequest).get("id");
        System.out.println(serviceClass+" "+idClass);
        Method method = ReflectionUtils.findMethod(serviceClass, "find",idClass);
        System.out.println(method+"  "+service+" "+id+" "+req);
        
        Long idVal = Long.valueOf(idStr);
        Object obj = ReflectionUtils.invokeMethod(method, service, idVal);//method:find Service:DonorRequestService id:16
        System.out.println(obj+"request is "+req);
